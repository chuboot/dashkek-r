  ...
  useEffect(() => {
    fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vR_ECkK7QC1cAwstl9nqLf-HA3ySzXySjugSqVCsXiendHwEun3giJBH6SyIuiQR9M63K_IR6FQ2UYH/pub?output=csv')
      ...
      .then((data) => {
        ...
        let filteredRowCount = 0; // Initialize count for filtered rows
        ...
        const locationColumnIndex = headers.indexOf('LokasiKEK');
                ...
                // Memfilter baris berdasarkan kolom "LokasiKEK" == "KEKSanur"
                for (let i = 1; i < rows.length; i++) {
                    ...
                    if (values.length > locationColumnIndex) {
                        const locationValue = values[locationColumnIndex].trim().toLowerCase(); // Trim and convert to lowercase
                        if (locationValue === 'keksanur') { // Compare with lowercase target
                            filteredRowCount++;
                        }
                    }
                }
        ...
        // setJumlahPelakuUsaha(values.length);
        setLoading(false);
      })
      .catch((error) => {
